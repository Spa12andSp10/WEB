<?php
    ...
    if ($_POST) {
        ...
        elseif (!preg_match($patter_name, $_POST['name'])) {
            $errors['name'] = 'Проверьте наличие лишних символов в имени! <br>';
        }
        if (empty($_POST['lastname'])) {
            $errors['lastname'] = 'Поле с фамилией обязательно к заполнению! <br>';
        }
        elseif (!preg_match($patter_lastname, $_POST['lastname'])) {
            $errors['lastname'] = 'Проверьте наличие лишних символов в фамилии! <br>';
        }
        ...
        if (!$errors) {
            $date = new DateTime();
            $for_result = $date->format('Y-m-d H:i:s');
            $result = implode('|', $_POST);
            $result = $result . "|" . $_SERVER['REMOTE_ADDR'] . "|" . $for_result . "\n";
            $file = "./data/data.txt";
            if(!file_exists($file))
                {
                    touch("data.txt");
